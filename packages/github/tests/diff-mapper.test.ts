...
describe("diff-mapper", () => {
  it("should map a single line addition correctly", () => {
    ...
    expect(result).toEqual({ kind: "inline", path: "test.ts", line: 11, side: "RIGHT", start_line: undefined, start_side: undefined });
  });
  ...
  it("should map a single line deletion correctly", () => {
    ...
    expect(result).toEqual({ kind: "inline", path: "test.ts", line: 12, side: "LEFT", start_line: undefined, start_side: undefined });
  });
  ...
  it("should demote finding if outside patch hunks", () => {
    ...
    expect(result).toEqual({ kind: "demoted", reason: "Line 100 in test.ts is outside the patch hunks" });
  });
  ...
  it("should handle multiline additions correctly", () => {
    ...
    expect(result).toEqual({ kind: "inline", path: "test.ts", line: 12, side: "RIGHT", start_line: 11, start_side: "RIGHT" });
  });
  ...
  it("should demote if no locationHint", () => {
    const set = new CommentableSet([{ filename: "test.ts", status: "added", patch: "@@ -0,0 +1,1 @@\n+added" }]);
  ...
  it("should handle deleted file (no RIGHT side lines)", () => {
    ...
    expect(result).toEqual({ kind: "inline", path: "test.ts", line: 2, side: "LEFT", start_line: 1, start_side: "LEFT" });
  });
});
